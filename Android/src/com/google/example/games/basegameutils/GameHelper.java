...
import com.google.api.games.Extension;
import com.google.api.games.SignInActivity;
...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks,
    ...
    private boolean isStartCallbackReturned = false;
    ...
    Activity mActivityForDialogs = null;
    ...
    public void onStart(Activity act) {
    	...
    	if(isStartCallbackReturned){
    		mActivity = act;
    		continueUserInitiatedSignIn();
    ...
    public void beginUserInitiatedSignIn() {
        ...
        Intent hide = new Intent(mActivity,mActivity.getClass());
		
		mActivity.startActivity(hide);
        

       
    }
    
    private void continueUserInitiatedSignIn(){
    ...
    public void onDisconnectedFromGoogleActivity(){
    	if(mListener != null){
    	 mListener.onSignInFailed();
    	}
    ...
    public Dialog makeSimpleDialog(String text) {
        ...
        Dialog dialog;
        if(!mActivity.isTaskRoot())
        dialog = makeSimpleDialog(mActivityForDialogs, text);
        else 
        	dialog = makeSimpleDialog(mActivity, text);
        return dialog;
    }

    public Dialog makeSimpleDialog(String title, String text) {
        if (mActivityForDialogs == null) {
            logError("*** makeSimpleDialog failed: no current Activity!");
            return null;
        }
        Dialog dialog;
        if(!mActivity.isTaskRoot())
        dialog = makeSimpleDialog(mActivityForDialogs,title, text);
        else 
        	dialog = makeSimpleDialog(mActivity,title, text);
        return dialog;
    }
